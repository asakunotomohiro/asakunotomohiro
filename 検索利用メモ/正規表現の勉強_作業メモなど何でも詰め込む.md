...
* 文字クラス  
  ...
  別の書き方として、`[a-e]`でも同じ意味(これを文字クラスという)。  
  この場合、ハイフン記号にマッチしないため、マッチさせる場合は、`[a\-b]`・`[-abc]`・`[abc-]`のように、エスケープ・先頭・末尾にすること。  
  そして、`[.]`のドット記号は、ワイルドカードではなく、ドットそのものにヒットする。  
  例）[**3.14**]の場合、数字の3・ドット記号・数字の1・数字の4のどれかにヒットする。  
  他の特殊記号として、`[\n]`は改行、`[\t]`はタブを表す。  
  その他にも[ブラケット表現](#regularLanguageGeneraltermBracketexpression)を参照すること。  
...
※ここで言うワードというのは、Perlの変数や関数に使える文字列を表す。  
...
* 量指定子  
  ...
  * `*`：直前項目が0回以上現れることを意味する(同じ項目である限りマッチし続ける)。  
    強欲(より多くのものにマッチするように動く・**欲張り**とも言う)。  
...
|`*?`|0回以上、できるだけ少なく。|
|`+?`|1回以上、できるだけ少なく。|
|`{最小出現回数, }?`|最低でも最小出現回数だが、できるだけ少なく。|	←☆どういう意味？以下ってこと？
|`{最小出現回数, 最高出現回数}`|最低でも最小出現回数であり、多い場合は最高出現回数まで。|
|`{指定出現回数}?`|指定した出現回数のみ|

<details><summary>Perl言語での使用例）</summary>

```perl
use v5.24;

my $feeling = '本日は晴晴晴晴天なり。';
my $today = 20330210;

if( $feeling =~ /晴/p ) {
	# pフラグを付けた場合、以下の検索にマッチした部分のみを抜き出す変数が有効化される。
	say "晴\t${^MATCH}";
	# 検索結果：晴
}
if( $feeling =~ /晴*/p ) {
	say "晴*\t${^MATCH}";
	# 検索結果：晴	←☆想定では、後ろに続く"晴"の文字列全てマッチすると思っていた。
}
if( $feeling =~ /晴+/p ) {
	say "晴+\t${^MATCH}";
	# 検索結果：晴	←☆想定では、後ろに続く"晴"の文字列全てマッチすると思っていた。
...
if( $today =~ /3*/p ) {
	say "3*\t${^MATCH}";
	# 検索結果：空文字列(undef)だが、検索にはマッチしたようだ。
}
if( $today =~ /3+/p ) {
	say "3+\t${^MATCH}";
	# 検索結果：33
...
todo:
0回以上や1回以上の言葉の意味を理解していないようで、検索結果が想定外になっている。  
それを調べる。  
...
* 部分正規表現  
  ...
  * キャプチャ  
    ...
    [Perl](#regularLanguageGeneralTermsLanguageperlcapture)の場合、`/(\d+)/`の場合は1桁以上の数字にマッチしたひとくくりのものが`$1`にキャプチャされている。
  ...
  * クラスタ化  
    ...
    キャプチャすることの利点は、同じ単語を使い回せることにある。  
    例）`(hoge)\1`の場合、**hogehoge**が検索に掛かる。  
    複数利用可能：`(a)(z)\2\1`の場合、**azza**が検索に掛かる。  
    そして、Perlでは、`(?:パターン)`と記述することで、キャプチャ(例えば`\1`)から外れる。  
    また、キャプチャ番号をPerlで使う場合は、**\1**などではなく、**\g{N}** を用いるのが吉。  
  ...
  * 後方参照  
    ...
    例）"hello"の文字列に`e(l)\1o`とした場合、**ello**としてヒットする(`\1`は、直前の括弧内の文字を指している)。  
...
* アンカー  
  正規表現がマッチする位置を特定の場所に固定するために用いる。  
  その特定場所を固定するのが強制であるため、ゼロ幅言明(もしくは、ゼロ幅アサーション：zero-width assertion)とも言う。  
  また、特定の条件にマッチするが、文字にはマッチしない(位置にマッチさせるため)。  
  例えば、Perlの`\A`**(文字列先頭)** で検索する場合、先頭文字にマッチしなければ、その1行内で文字をずらさずに終了する(オプションによっては次の行に移る)。  

<a id="regularLanguageGeneraltermAnchorpeculiar"></a>
<details><summary>アンカーは、独自に作られている部分があり、個別に調べた方が良い</summary>

  * アンカー  
    <a id="regularLanguageGeneraltermAnchorpeculiarcaret"></a>
    * [`^`](#regularLanguageGeneraltermAnchorpeculiarcaretprogram)  
      ハットもしくはキャレットと読む。  
      行終端子の直後にマッチ(`\A`とは異なる)または文字列の先頭。  
      vimでは、`\_^`と言うのも存在する。  
      Perlなどのマルチライン`/m`修飾子を付けた場合、`\A`と同じ働きとして改行直後にマッチする(標準では行先頭を表す)。  
    <a id="regularLanguageGeneraltermAnchorpeculiaruppercasea"></a>
    * [`\A`](#regularLanguageGeneraltermAnchorpeculiaruppercaseaprogram)  
      文字列の先頭にのみマッチ。  
      使える処理系は少ない(Perlはv5から使える)。  
    <a id="regularLanguageGeneraltermAnchorpeculiardollarsign"></a>
    * [`$`](#regularLanguageGeneraltermAnchorpeculiardollarsignprogram)  
      行終端子の直前にマッチ(`\z`・`\Z`とは異なる)または文字列の末尾。  
      vimでは、`\_$`と言うのも存在する。  
      Perlなどのマルチライン`/m`修飾子を付けた場合、`\Z`と同じ働きとして改行文字直前にマッチする。  
    <a id="regularLanguageGeneraltermAnchorpeculiaruppercasez"></a>
    * [`\Z`](#regularLanguageGeneraltermAnchorpeculiaruppercasezprogram)  
      文字列の末尾あるいは文字列の末尾の行終端子の直前にマッチ(改行含む)。  
      使える処理系は少ない(Perlはv5から使える)。  
    <a id="regularLanguageGeneraltermAnchorpeculiarlowercasez"></a>
    * [`\z`](#regularLanguageGeneraltermAnchorpeculiarlowercasezprogram)  
      完全な文字列の末尾にマッチ(改行すらない末尾)。  
      使える処理系は少ない(Perlはv5から使える)。  
    <a id="regularLanguageGeneraltermAnchorpeculiarlowercaseb"></a>
    * [`\b`](#regularLanguageGeneraltermAnchorpeculiarlowercasebprogram)  
      単語の境界にマッチ。  
      使える処理系は少ない(Perl[独自](#regularLanguageGeneraltermAnchorpeculiarlowercasebtable)がある)。  
      `\w\W`の組み合わせと同じ(この場合は、単語終了境界になり、`\W\w`の場合は単語開始境界になる)。  
    <a id="regularLanguageGeneraltermAnchorpeculiarlessthansign"></a>
    * `\<`  
      単語の先頭にマッチ。  
      使える処理系は少ない。  
    <a id="regularLanguageGeneraltermAnchorpeculiaruppercaseb"></a>
    * [`\B`](#regularLanguageGeneraltermAnchorpeculiaruppercasebprogram)  
      単語の境界以外にマッチ。  
      使える処理系は少ない。  
    <a id="regularLanguageGeneraltermAnchorpeculiargreaterthansign"></a>
    * `\>`  
      単語の末尾にマッチ。  
      使える処理系は少ない。  
    <a id="regularLanguageGeneraltermAnchorpeculiaruppercaseg"></a>
    * `\G`  
      前回のマッチの末尾にマッチ。  
      使える処理系は少ない。  
    <a id="regularLanguageGeneraltermAnchorpeculiarlowercasebparenthesesx"></a>
    * `\b{X}`  
      Unicodeの書記素クラスタ／単語／文の境界にマッチ。  
      使える処理系は少ない。  
      基本的には、Perlだけ？  

<a id="regularLanguageGeneraltermAnchorpeculiarlowercasebtable"></a>
以下、Perl限定(ファンシーなワード境界)。
|ワード境界|意味|対応バージョン|
|----------|----|--------------|
|`\b`|通常のワード境界|Perl|
...
Perlでの検索(`m/パターン/`)時に、`/m`修飾子を付ける(`m/パターン/m`)ことで、文字列中の行頭や改行文字にもマッチする。  
付けなければ、文字列全体(ファイル内)の1カ所のみヒットする(先頭と末尾のみヒットする・各行頭や改行はヒットしない)。  

<details><summary>Perlで使う文字列。</summary>

以下、文字列。
```perl
my $helloworld = << "STRING";
Hello
こんにちはwWorld
(
$^V
...
<a id="regularLanguageGeneraltermAnchorpeculiarcaretprogram"></a>
以下、`^`記号を使った結果(m修飾子付き)。
```perl
use v5.24;

if ( $helloworld =~ /^World/m ) {	←☆m修飾子を付けているため、検索にかかる(各行の行頭を検索しているため)。
	say "[$&]";	# 出力結果：[World]
}
```
m修飾子を付けたことで、複数行として(改行を改行として認識)検索することにより、行先頭アンカーとして機能する。  

以下、`^`記号を使った結果(m修飾子なし)。
```perl
use v5.24;

if ( $helloworld =~ /^World/ ) {	←☆文字列の先頭にないため、検索には掛からない(行の先頭ではある)。
	say "[$&]";	←☆この行出力されない。
}
```
改行はただの文字列として扱うため、**\A**と同じ働きをする。  
しかし、大規模開発で誰かがm修飾子を付けた場合に挙動が変わってしまうため、意図した結果を得るためにも**\A**との使い分けをすること(できることならば、コメントで説明しておくのが良いかもしれない)。  

<a id="regularLanguageGeneraltermAnchorpeculiaruppercaseaprogram"></a>
以下、`\A`記号を使った結果。
```perl
use v5.24;

if ( $helloworld =~ /\AWorld/ ) {	←☆文字列の先頭にないため、検索には掛からない(行の先頭ではある)。
	say "[$&]";	←☆この行出力されない。
}
```
上記のm修飾子なしの`^`と同じ結果になった。  
m修飾子有無に左右されずに動くため、本当に文字列先頭を検索する場合は、**\A**を使った方が良い。  

<a id="regularLanguageGeneraltermAnchorpeculiardollarsignprogram"></a>
以下、`$`記号を使った結果(m修飾子付き)。
```perl
use v5.24;

if ( $helloworld =~ /World$/m ) {	←☆m修飾子を付けているため、検索にかかる(各行の末尾を検索しているため)。
	say "[$&]";	# 出力結果：[World]
}
```
...
if ( $helloworld =~ /World$/ ) {	←☆文字列の末尾にないため、検索には掛からない(行の末尾ではある)。
	say "[$&]";	←☆この行出力されない。
}
```

<a id="regularLanguageGeneraltermAnchorpeculiaruppercasezprogram"></a>
以下、単語末尾に`\Z`記号を使った結果。
```perl
use v5.24;

if ( $helloworld =~ /World\Z/ ) {	←☆行の末尾であるため、検索にかかるはずだが、かからない。
	say "[$&]";	←☆この行出力されないのだが、それはおかしい。
}
```
~~末尾の改行有無を気にせず検索するはずだが、検索に掛からないのはあり得ない~~。  
~~なぜ、こんな結果になった？~~  
`/WorldD\Z/`であれば、検索に掛かった。  
要は、m演算子なしの **$** と同じ結果になるのが正解。  
そのため、m演算子の有無に左右されずに動く。  

<a id="regularLanguageGeneraltermAnchorpeculiarlowercasezprogram"></a>
以下、単語末尾に`\z`記号を使った結果。
```perl
use v5.24;

my $helloworld = "Hello\nこんにちはwWorld\n(\n$^V\n)\nWorld 今後もよろしく\n.\nWorldD\n";	←☆文字列の末尾にないため、検索には掛からない(行の末尾ではある)。
my $helloworld = "Hello\nこんにちはwWorld\n(\n$^V\n)\nWorld 今後もよろしく\n.\nWorldD";	←☆これにはかかる(末尾に改行が無いため)。

if ( $helloworld =~ /WorldD\z/ ) {	←☆文字列の末尾になるため、検索に掛かる。
	say "[$&]";
}
```
上記のm修飾子なしの`$`と同じ結果になった。  

ファイル内容などから取得する場合、末尾に改行が付いている場合がある。  
その場合は、**chomp**などを用いて削除後に検索を掛ける必要がある。  
m修飾子有無に左右されずに動くため、本当に文字列末尾を検索する場合は、**\z**を使った方が良い。  
もしくは、改行の有無に左右されず、**\Z**を使った方が良い。  

<a id="regularLanguageGeneraltermAnchorpeculiarlowercasebprogram"></a>
以下、単語末尾の[境界](#regularLanguageGeneraltermAnchorpeculiarlowercasebtable)に`\b`記号を使った結果。
```perl
use v5.24;

if ( $helloworld =~ /World\b/p ) {
	say "[${^PREMATCH}]";
	say "[${^MATCH}]";	←☆[World]が出力される("こんにちはwWorld"行の後半部分)。
}
```

以下、単語先頭の[境界](#regularLanguageGeneraltermAnchorpeculiarlowercasebtable)に`\b`記号を使った結果。
```perl
use v5.24;

if ( $helloworld =~ /\bWorld/p ) {
	say "[${^MATCH}]";	←☆[World]が出力される("World 今後もよろしく"行の前半部分)。
	say "[${^POSTMATCH}]";
}
```

<a id="regularLanguageGeneraltermAnchorpeculiaruppercasebprogram"></a>
以下、単語末尾の[境界以外](#regularLanguageGeneraltermAnchorpeculiarlowercasebtable)として`\B`記号を使った結果。
```perl
use v5.24;

if ( $helloworld =~ /World\B/p ) {
	say "[${^MATCH}]";	←☆[World]が出力される("WorldD"行の前半部分)。
	say "[${^POSTMATCH}]";
}
```
末尾にもワードが付いているため、検索に掛かった。  
非定型を使うのはややこしくなるため、あまり使いたくない。  

以下、単語先頭の[境界以外](#regularLanguageGeneraltermAnchorpeculiarlowercasebtable)として`\B`記号を使った結果。
```perl
use v5.24;
...
* エスケープ  
  ...
  Perlの場合、メタクォートとして`\Q`を起点としてそれ以降エスケープが自動で行われる(停止する場合は`\E`で止める)。  
  また、Perlでメタクォートを使うほかに、quotemeta関数というのも存在する。  
...
* 修飾子  
  別名：[**マッチ修飾子**](#regularLanguageGeneralTermsLanguageperl)[**(Perl用語)**](#regularLanguageGeneralTermsLanguageperlmatchmodifier)・**パターン修飾子(PHP)**・フラグ(flag)とも呼ぶ。  
...
<details><summary>Perl言語での`m//`をリストコンテキストで使う(splitと逆)</summary>

[split](#regularLanguageGeneralTermsLanguageperlDetailedsplit)では、不要文字を区切りパターンとして置換作業を実施した。  
今回は、必要な部分をパターン化して置換する。  

```Perl
my $hoge = "Hello World boo!";	# 通常利用の検索結果をスカラーコンテキストに代入している。
my ($hello, $world, $boo) = $hoge =~ m/(\S+) (\S+) (\S+)/;
say "$hello, $world, $boo";	# Hello, World, boo!

my $hoge = "3 shout 8 word ⇒ Hello World, asakuno!";	# 区切り作業処理開始。
my @words = $hoge =~ m/([a-z]+)/ig;	#	リストコンテキストとして値の代入。
say "@words";	# shout word Hello World asakuno	リストコンテキストとして表示。
```
split演算子とは逆に、残したい部分を指定する。  

</details>

* Perl独自の検索結果の特殊変数  
  サンプルあり(ファイル名：[Perl独自の特殊変数(正規表現用のみ).pl](Perl言語の勉強(正規表現利用に限る)/Perl独自の特殊変数(正規表現用のみ).pl))。  
  以下、都度説明しているが、p修飾子`[p]`が必要な自動マッチ変数がある。  
  * `$&`  
    マッチした文字列。  
...
キャプチャを使う場合は、条件式で成功可否を確認する必要があると言うこと。  
そして、成功した場合、マッチ変数を通常の変数に代入すること(`my $hoge = $g{1}`などにして、その後**$hoge**を使うこと)。  
...
* マッチ修飾子(match modifier)  
  ...
  * `[i]`(insensitive)  
    大小文字区別なし検索をする(insensitiveの頭文字)。  
    例）`if( $yesno =~ /yes/i )`  
  * `[m]`  
    文字列を複数行として扱う。  
    [`^`](#regularLanguageGeneraltermAnchorpeculiarcaretprogram)と[`$`](#regularLanguageGeneraltermAnchorpeculiardollarsignprogram)を文字列中の改行文字とマッチさせることになる。  
  * `[s]`(single line)  
    文字列を単独行として扱う。  
  ...
  * `[x]`(extend)  
    ...
    Perlのシャープ記号以降がコメントになるため、シャープ記号素のものの検索には`\#`とする必要がある(もしくは`[#]`)。  
...
* 文字の解釈フラグ  
  ...
  * `/a`  
    ...
    **A-Z, a-z, 0-9, _**  
    ...
    `/a`は文字クラスショートカットに影響させるだけだが、`/aa`の場合はASCIIに対してのみ行う。  
  * `/u`  
    Unicodeの文字を使う。  
    例）`/\w+/u`
    **任意のUnicodeワード文字**  
...
* 正規表現の優先順位  
  1. [グループ化](#regularLanguageGeneraltermGroupingconstructs)と[キャプチャ](#regularLanguageGeneralTermsLanguageperlcapture)に使われる括弧。  
     例）`(...)`・`(?:...)`・`(?<名前>...)`  
  1. [量指定子](#regularLanguageGeneraltermQuantifier)  
     例）`*`・`+`・`?`・`{X,Y}`・`{X,}`・`{X}`。  
  1. [アンカー](#regularLanguageGeneraltermAnchor)と並び(sequence)。  
     アンカー：`\A`・`\Z`・`\z`・`^`・`$`・`\b`・`\B`など。  
     並び：文字を並べた状態(例：**abc**など)。  
  1. 選択肢`|`。  
  1. アトム(atom)。  
     文字幅が0でないものを表す。  
...
* キャプチャ  
  マッチ変数とは、括弧付きの正規表現実行により、マッチした部分を変数に記録する。その変数をマッチ変数と言う(また、記録することをキャプチャすると言う)。  
  [スコープ(有効期限)](#regularLanguageGeneralTermsLanguageperlDetailedregnote2)あり。  
  * 通常キャプチャの[利用方法](#regularLanguageGeneralTermsLanguageperlDetailedregBackreference)(`$1`・`$2`・`$3`・・・)。  
    ...
    ※[後方参照](#regularLanguageGeneraltermBackreference)で使う`\1`とキャプチャとしての`$1`は、中身は同じになるだろうが、別の意味を持っているため、気をつけること(以下、それぞれの役割)。  
    * 後方参照`\数字`or`\g{数字}`  
      検索時に格納される一時変数だと思うこと。  
    * マッチ変数`$数字`  
      検索語に格納される変数だと思うこと。  
...
細かいサンプルは、[Perl用リードミーファイル](../Perl言語/README.md)参照([置換演算子](#practicaluseSubstitutedisplacement))。  
※ページ(リンク先)に飛べないはず。  
※大小置換も取り扱っている(`\U`・`\L`)(`\F`は無し)。  
...
* 置換修飾子
  ...
  `/m`
  `/s`が使える。  
  `/o`が使えない(代わりに、[メタクォート](#regularLanguageGeneraltermMetaquart)を使う)。  
  * `/g`  
    出現パターン全てを置き換える。  
  * `/e`  
    置換文字列をPerl式として評価する。  
...
また、例外として、カンマ区切りのCSVファイルを除く(単純置き換えしか出来ないため、CSVなどの複雑な区切りは対応できない。外部モジュールを使うべし)。  

様式：
`my @配列名 = split /区切りパターン/, $文字列変数名;`  
※"区切りパターン"が、本来欲しいデータ部分ではないことを意識する(極端な表現をするならば、切り捨てられる)。  
...
<a id="regularLanguageGeneralTermsLanguageperlDetailedsplitnote2"></a>
**注意事項2**：
区切りパターンが連続である場合は、空データ扱いされる。  
しかし、それは先頭や中間にある場合であり、末尾にある場合は切り捨てられる。  
切り捨てたくない場合は、第3引数に **-1** を指定すること。  

<a id="regularLanguageGeneralTermsLanguageperlDetailedsplitnote3"></a>
**注意事項3**：
単体で使う場合、**$_** に対して、半角スペース区切りが行われる。  
例）`my @配列名 = split;`  
この場合は、**$_** に格納されている文字列の先頭に半角スペースがある場合は、切り捨てられる([注意事項2](#regularLanguageGeneralTermsLanguageperlDetailedsplitnote2)が適用外)。  
...
`my $rejoin = join ":", 2, 4, 8, 16, 32, 64, 128;`  
※**$rejoin**：**2:4:8:16:32:64:128**

<a id="regularLanguageGeneralTermsLanguageperlDetailedjoinfunknote1"></a>
**注意事項1**：
**$つなぎ文字用変数名**は、文字列であって、パターンでは無いことに注意する。  
配列内容(もしくはリスト)をつなぐだけなので、先頭や末尾の要素には効果が無い。  

<a id="regularLanguageGeneralTermsLanguageperlDetailedjoinfunknote2"></a>
**注意事項2**：
**$つなぎ文字用変数名**は、文字列であって、パターンでは無いことに注意する。  
大事なことなので、2回発言した。  
...
$ perl -p -w -e 'chomp(my $date = ``date``);' -e 's/Date:.*/Date: $date/' hoge*.md	←☆`をどうやってエスケープする？
...
<details><summary>Perl言語での文字置き換え(tr変換演算子)の細かい話</summary>
...
#### . 任意の1文字にマッチ
ワイルドカードのひとつ。  
メタキャラクタ(metacharacter)のひとつでもある。  

基本的には、どのような1文字にもマッチする。  
しかし、唯一マッチしない文字が2種類あり、それが改行(**LF: U+000A**)とNUL(**U+0000**)の2つ。  
また、ドット記号そのものにヒットさせる場合は、エスケープさせる必要がある(`\.`)。  

以下、言語ごとの細かい方言(違い)を説明する。  
<details><summary>目次</summary>

* 一覧  
  * [Vim](#regularDesktopReferenceVim3030103)  
  * [秀丸](#regularDesktopReferenceHidemaru3030103)  
  * [サクラエディタ](#regularDesktopReferenceSakura3030103)  
  * [grep](#regularDesktopReferenceGrep3030103)  
  * [Go](#regularDesktopReferenceGo3030103)  
  * [Perl](#regularDesktopReferencePerl3030103)  
  * [AutoHotKey](#regularDesktopReferenceAutohotkey3030103)  
  * [C#](#regularDesktopReferenceCSharp3030103)  
  * [egrep](#regularDesktopReferenceEgrep3030103)  
  * [sed](#regularDesktopReferencesed3030103)  
  * [awk](#regularDesktopReferenceAwk3030103)  
  * [Java](#regularDesktopReferenceJava3030103)  
  * [Python](#regularDesktopReferencePython3030103)  
  * [Vim Script](#regularDesktopReferenceVimscript3030103)  
  * [Godot](#regularDesktopReferenceGodot3030103)  
  * [C](#regularDesktopReferenceClang3030103)  
  * [C++](#regularDesktopReferenceCplusplus3030103)  
  * [Rust](#regularDesktopReferenceRust3030103)  
  * [Tcl](#regularDesktopReferenceTcl3030103)  
  * [.Net](#regularDesktopReferenceDotNet3030103)  
  * [JavaScript](#regularDesktopReferenceJavascript3030103)  
  * [Ruby](#regularDesktopReferenceRuby3030103)  
  * [PHP](#regularDesktopReferencePhp3030103)  
  * [SQL](#regularDesktopReferenceSql3030103)  
  * [Visual Basic](#regularDesktopReferenceVisualbasic3030103)  
  * [VBScript](#regularDesktopReferenceVbscript3030103)  
  * [ECMAScript](#regularDesktopReferenceEcmascript3030103)  
  * [elisp](#regularDesktopReferenceElisp3030103)  
  * [ライブラリ](#regularDesktopReferenceLibrarie3030103)  

<a name="regularDesktopReferencePerl3030103"></a>
##### Perl
Perl限定のルールはほぼない。  
基本的には、改行とNull以外にヒットする。  
また、v5.12に、`\N`がドットワイルドカードと同等の機能を持った文字クラスのショートカットが追加された(`date\N`)。  
そして、ドットそのものにヒットさせたい場合は、他の正規表現と同じように、エスケープさせる必要がある(`/date\./`)。  

以下、プログラム。
```perl
use v5.24;

my @reg = qw(	←☆コメントを付けるために、勝手に改行している。
        date.!	←☆ヒットする。
        date.?	←☆ヒットする。
        date:	←☆ヒットする。
        date	←☆ヒットしない。
