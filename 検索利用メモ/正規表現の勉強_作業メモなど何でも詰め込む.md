...
その理由は、上記エディタの[Vim](https://vim-jp.org/vimdoc-ja/pattern.html#perl-patterns)・[サクラエディタ](https://sakura-editor.github.io/help/HLP000006.html)の3種類のうち2種類がPerl互換の正規表現対応している。  
だからなんだって話だが・・・。  
...
* POSIX文字クラス表現(POSIX character class expression)  
...
* ダイアクリティカルマーク(diacritical marks)  
  アクセント記号のこと。  
  通常の文字との合字で表現する。  
  `ä`・`℅`・`©`など。  
...
<details><summary>独自に作られているため、個別に調べた方が良い</summary>
  ...
  * アンカー  
    * `^`  
      文字列の先頭または行終端子の直後にマッチ(`\A`とは異なる)。  
    * `\A`  
      文字列の先頭にマッチ。  
      使える処理系は少ない。  
    * `$`  
      文字列の末尾または行終端子の直前にマッチ(`\z`・`\Z`とは異なる)。  
    * `\Z`  
...
* 基本正規表現(basic regular expression：BRE)  
...
* 拡張正規表現(extended regular expression：ERE)  
  POSIX定義の2種類のうちの1つ(単純正規表現は含まない)。  
...
* 検索種類。  
  * Perl(Python・Ruby)検索に近くする。  
    `/\v`(verymagic検索)  
    **_**・**大小文字のアルファベット**・**0〜9**を除く全ての文字が特殊な意味を持つ。  
  ...
  * テキストそのものの検索。  
    `/\V`(verynomagic検索)  
    ※バックスラッシュ記号だけが特殊な意味を持ち、それ以外は見たままを検索する。  
    ※検索単語に`/`が含まれていた場合、[エスケープ](#regularLanguageGeneralTermsLanguagevimEscape)する必要がある(それ以降の文字を検索対象から外すため)。  
  * 本来のvim検索。  
    `/`(magic検索)  
  * vi挙動検索。  
...
1. [メタキャラクタリファレンス](#regularDesktopReference3000001)
   ...
   1. [CANON_EQフラグ 等価とみなされる文字を同じ文字としてマッチ](#regularDesktopReference3030815)  
...
1. [逆引きリファレンス 置換編](#regularDesktopReference6000001)
   ...
   1. [「product_name」を「productName」に変換したい](#regularDesktopReference6060302)  
   1. [「PRODUCT_NAME」を「productName」に変換したい](#regularDesktopReference6060303)  
...
ちなみに、vimは、[NFAエンジン](https://vim-jp.org/vimdoc-ja/pattern.html)を使っているようだ(で、それは何？)。  
...
#### 「product_name」を「productName」に変換したい
...
#### CANON_EQフラグ 等価とみなされる文字を同じ文字としてマッチ
