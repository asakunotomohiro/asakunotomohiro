...
まずは、チャット画面に入る。  
通常利用：<https://web.libera.chat>  
アカウント作成：<https://web.libera.chat/gamja/>  
...
```
このメッセージが出たときはログインせずに同じ名前のユーザでログインしている人がいるのが原因(多分自分自身)。  
...
もし、ゲストユーザのままの場合、ユーザ情報は、`~asakunotomohiro@192.168.0.1`のように、他者に自分の居場所が判明した状態になる(だから何だって話だが)。  
今回、アカウントを作成したことにより、`~asakunoto@user/asakunotomohiro`となった。  
アットマーク記号前の文字列に不快感が・・・(文字数制限だろうが、何か説明が欲しかったよ)。  
これであれば、名字だけにすればよかった。  
