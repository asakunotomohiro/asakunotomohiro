...
※[日本Androidの会](https://www.android-group.jp)があることを始めて知ったよ([@Twitter](https://twitter.com/androidgroup_jp))。  
...
古いが、未だに使っている。  
家から持ち出さず、しかもWiFi運用のみであるため、あまり使っているとは言いがたいが・・・。  
...
※当時の状況を覚えていないのだが、microSDカードを挿したままOSバージョンを上げてしまったため、壊れたと思っている(9/17)。  
...
<details><summary>作業記録(無駄な作業だった)。</summary>
...
```terminal
$ ./adb shell	←☆Pathを通していないため、カレントディレクトリにあることを示して実行。
...
$
```
どうやら、Adroid10向けに対応できないのが原因で、今後はGooglePlayからインストールしたTermuxは使い物にならないそうだ。  
残念だ。  
...
それでも無法地帯は変わらないため、あるからという理由でバカチョン導入したらえらい目に遭うだろう。  
そして、[GooglePlay](https://play.google.com)の場所にあるソフトウェアも安全が保証されているわけでは無いため、どっこいどっこいとも言える。  
欲しいのは自己責任で手に入れる必要があり、自己責任で使う必要がある。  
...
※[設定]画面-[シェル]画面にある。  
今見返したのだが、**v9.0.0363**では、このボタンが無くなっている。  

そして、内蔵ではなく、外部SDカード(microSD)などに対して有効なのかもしれない。  
内蔵だけの場合、ドキュメントディレクトリやダウンロードディレクトリなどに接続できない。  
...
<details><summary>curlを使ったPerlインストール？</summary>

curlが使えるため、Perlをインストールできると思ったのだが、出来なかった。  

```terminal
$ curl -fsSL https://git.io/perl-install | bash -s perl
Installing perl 5.36.0.1 to /data/user/0/com.droidvim/files/home/perl, this may take a while...
Successfully installed perl 5.36.0.1.
To use this perl, add the following line to ~/.bash_profile or ~/.zshrc:

  export PATH=/data/user/0/com.droidvim/files/perl/bin:$PATH

$
```
圧縮されたURLのようで、中身はbashからcurlを実行するプログラムになっている。  
中身を見ずに実行してしまったよ・・・悪意あるサイトで無くて良かった(と思っておこう)。  
よく見ていないが、きっと大丈夫だろう。  
