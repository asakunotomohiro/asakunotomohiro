...
以下、無理矢理`\Z`で検索に掛かるようにした。
```perl
use v5.24;

my $helloworld = "Hello\nこんにちはwWorld\n(\n$^V\n)\nWorld 今後もよろしく\n.\nWorldD";	←☆これも検索に掛からない。
my $helloworld = "こんにちはwWorld\n";
if ( $helloworld =~ /World\Z/ ) {
	say "[$&]";
}
```
これならば検索に掛かったが・・・不満(当たり前だが`\z`では検索に掛からない)。  
変数に複数行を入れているのが原因か？  
...
if ( $helloworld =~ /World\z/ ) {	←☆文字列の末尾にないため、検索には掛からない(行の末尾ではある)。
	say "[$&]";	←☆この行出力されない。
}
```
上記のm修飾子なしの`$`と同じ結果になった。  
m修飾子有無に左右されずに動くため、本当に文字列末尾を検索する場合は、**\z**を使った方が良い。  
変数に複数行を入れているため、失敗している？  
