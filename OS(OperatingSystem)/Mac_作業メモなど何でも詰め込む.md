...
<details><summary>工場出荷状態に戻す手順(---簡単---)。</summary>

* 工場出荷状態に戻す手順。  
  1. **システム設定**を開く。  
  1. 左ペインから**一般**を開く。  
  1. 右ペインから**転送またはリセット**を開く。  
  1. 下部ボタンの**すべてのコンテンツと設定を消去**を開く。  
  1. ポップアップウィンドウの**消去アシスタント**にパスワードを入力する。  
  1. **ロックを解除**ボタンをクリックする。  
  1. **続ける**ボタンをクリックする。  
  1. **すべてのコンテンツと設定を消去**ボタンをクリックする。  
  1. 再起動するまで待つ。  
  1. WiFiに接続する。  
  1. アクティベートのために再起動する。  
  1. 工場出荷状態に戻ることを確認して、作業完了。  

</details>

* 工場出荷状態に戻す手順。  
  * 参考：[公式ページの説明引用](#macOperatingSystemreinstallsiliconOfficialexplanation)。  
  1. macOS Ventura 13以降の手順を実行。  
     1. 画面の左上にあるAppleメニュー  から「**システム設定**」を選択する。  
     1. サイドバーの「**一般**」をクリックする。  
     1. 右側ペインで下にスクロールして「**転送またはリセット**」をクリックする。  
     1. 「**すべてのコンテンツと設定を消去**」をクリックする。  
  1. 消去アシスタントが開く。  
     1. 管理者の資格情報でサインインを求められるため、Macへのログインに使っているパスワードを入力する。  
  1. 無効化または消去されるすべての設定・メディア・データ・その他の項目の要約が表示される。  
     ※ほかのAppleデバイスの設定やコンテンツに影響は及ばない(ローカル端末から削除されるだけ)。  
     1. 「**続ける**」をクリックする。  
  1. WiFiを選ぶように画面が出てきた。  
     * **Activate Mac**ということは、ライセンス自体が消えていると言うことだろうか。  
     * 認証終了後再起動が行われる(時間経過により自動再起動だが、手動で即再起動が可能)。  
  1. 工場出荷状態に戻ることを確認して、作業完了。  

愚痴：工場出荷状態にするためには、一旦ネットワークに接続してアクティベーション(ライセンス認証？)作業が必要であるため、巨大企業(GAFAM?)から離れるための[GrapheneOS](./GrapheneOS_作業メモなど何でも詰め込む.md)を選んだのが無駄になるという・・・。  
そして、工場出荷状態にしたあとは、普通にOSの利用を始めるのだが、そのときもWiFiに接続する必要がある(二度手間やん)。  

<details><summary>失敗作業記録(OS再インストールだが、既存環境は変わらず、OSだけが変わるのか？)。</summary>

以下の**macOSを再インストール**(**Tahoe**)を実行したが、まっさらな状態にならなかった。  
...
> 5. パスワードがわかっているユーザーを選択するよう求められた場合は、そのユーザーを選択し、「次へ」をクリックして、そのユーザーがこのMacにログインするために使っているパスワードを入力します。パスワードを忘れた場合は、こちらの記事を参照してください。  
> 6. Macが復旧から正常に起動すると、以下のようなユーティリティがウインドウに表示されます。
     * Time Machineから復元  
     * macOSを再インストール  
     * ディスクユーティリティ（起動ディスクを修復または消去するときに使用）  
     * 画面上部のメニューバーからは、ターミナルなどのほかのユーティリティを利用できます。  
> 7. 復旧を終了するには、Appleメニュー  から「再起動」または「システム終了」を選択します。  

<details><summary>失敗作業記録(詳細)。</summary>

※macOS再インストール手順だが、現行OSの入れ替えのような感じ？  
...
<a id="macOperatingSystemreinstallsiliconOfficialexplanation"></a>
<details><summary>公式ページの説明引用。</summary>
...
* 手順  
  ...
  * 参考：[Macのすべてのコンテンツと設定を消去する](https://support.apple.com/ja-jp/HT212749)  
  1. お使いのMacに搭載されているmacOSを確認して、適切な手順を実行します。  
     * **macOS Ventura 13以降**：画面の左上にあるAppleメニュー  から「**システム設定**」を選択します。  
       サイドバーの「**一般**」をクリックし、右側で下にスクロールして「**転送またはリセット**」をクリックします。  
       「**すべてのコンテンツと設定を消去**」をクリックします。  
     * **macOS Monterey 12**：画面の左上にあるAppleメニュー  から「**システム環境設定**」を選択します。  
       メニューバーの「**システム環境設定**」メニューから「**すべてのコンテンツと設定を消去**」を選択します。  
     * **macOS Big Sur 11以前**：~~「**すべてのコンテンツと設定を消去**」を使えない場合の対処法について、こちらを参照します~~(私に関係ないため公式ページを参照すること)。  
  1. 消去アシスタントが開きます。  
     管理者の資格情報でサインインするように求められたら、Macへのログインに使っているパスワードを入力します。  
  1. Macをリセットする前に[Time Machineでバックアップを作成する](https://support.apple.com/ja-jp/104984)かどうかを確認される場合があります。  
  1. 無効化または消去されるすべての設定、メディア、データ、その他の項目の要約が表示されます。  
     これによって、ほかのAppleデバイスの設定やコンテンツに影響が及ぶことはありません。  
     「**続ける**」をクリックします。  
  1. Apple AccountやAppleのサービスからのサインアウトを求められた場合は、Apple Accountのパスワードを入力します。  
     Apple Accountのパスワードを忘れた場合は、[こちらの記事をご確認ください](https://support.apple.com/ja-jp/102656)。  
     iCloudやApple Accountを使い続けるためにサインインし直すよう促すアラートが表示されても、無視してかまいません。  
  1. 「**すべてのコンテンツと設定を消去**」ボタンをクリックし、消去を確定します。  
  1. Macが再起動します。  
     起動時に次の操作を行います。  
     * キーボードやマウスなどのBluetoothアクセサリの電源を入れるように求められる場合があります。  
       アクセサリが30秒以内に接続されない場合は、アクセサリの電源をいったん切ってから入れ直してください。  
     * Wi-Fiネットワークを選択するように求められる場合があります。  
       画面の隅にあるWi-FiメニューWi-Fiメニューを使用してWi-Fiネットワークを選択できます。  
  1. Macがアクティベートされ、もう一度再起動します。  
     Macをはじめて設定するときと同様に、設定アシスタントが設定プロセスを案内してくれます。  
     Macを売却または譲渡したり下取りに出したりするため、出荷時の状態のままにしておきたい場合は、設定アシスタントを使わないでください。  
     以前Bluetoothデバイスを接続していた場合、それらのデバイスも使わないようにします。  
     そのままMacの電源ボタンを、電源が切れるまで押し続けてください。  

</details>

<details><summary>上記を私用に簡略記述した内容(数年前の作業であるため、かなり古い)。</summary>
