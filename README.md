...
<a name="HowToUseTheBranchOperationschedule"></a>
### 作業予定・残作業・完成予定など
mainブランチから勉強用に支流ブランチを作成する。  
その支流ブランチで勉強するのだが、さらに支流ブランチを作成する場合、本来の支流ブランチでの作業を忘れないために、**ToDo:**を付け、戻ってきたときに作業を忘れずすぐに取りかかれるようにしておくこと。  

以下、ToDoを記述するが、有効化ならず。
```markdown
todo: すぐに取りかかる作業を記載する。
    [ ] チェックリストとの併用は構わないが、
    [x] チェックリストだけでは心許ない。
    *   ゆえに、目立ちやすく、検索に掛かりやすいタグ(?)がマークダウンにあればいいのだが・・・。
```
Perlのスイッチ構文を調べていたことにより、その前のモジュールの勉強内容を完全に忘れたため、思い出すためにはチェックリストではなく、検索掛けやすく、忘れにくく、思い出しやすい方法を採用したいのだが・・・。  
何かないかな・・・。  


以下、[Qiita](https://qiita.com/Qiita/items/c686397e4a0f4f11683d)の補足説明を用いたいが、通常エディタでは対応していないからな・・・。
```markdown
:::note info
インフォメーション
infoは省略可能です。
