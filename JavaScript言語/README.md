...
本来の名前：[LiveScript言語](http://livescript.net)  
現在、その名前は、[CoffeeScript言語](https://coffeescript.org)からの派生だそうだ。  

**ECMA International**により規格が統一された。  
標準化されたJavaScriptを[ECMAScript](https://www.ecma-international.org)と呼ぶようだ。  
今回、2015年6月にECMAで公開されたECMAScript(ES2015・ES6)を基準に勉強を進めたいが、違う箇所の見極め方法が分からない。  


<a name="algorithmHowToUseTheBranch"></a>
## ブランチの利用方法
現在、構造化プログラミングの勉強を終えた。  
そのため、次に取りかかるのは、アルゴリズムの勉強になる・・・だろう。  
...
[コンピュータ用Opera](https://www.opera.com/ja/computer/neon)ブラウザは使わないかな・・・。  
デバッグには、[Google開発用Chrome](https://www.google.com/intl/ja_jp/chrome/)ブラウザを使う？  
...
* 以下、手順。  
  ...
  * [x] 手順2. ルートディレクトリのひな形ディレクトリをコピーしてくる。  
    ...
    ※その配下の**Pythonで学ぶアルゴリズムの教科書 一生モノの知識と技術を身につける**ディレクトリは削除。  
...
* 変数  
  * 変数宣言`let`  
    無くても問題ないが、付ける癖を無意識に行えるようにすること。  
    旧式は、`var`だったが、古いため、使わないこと(グローバル変数扱いになる)。  
  * 命名規則：キャメル記法  
    * 大小文字の区別あり。
    * 先頭に数字を使わない。  
