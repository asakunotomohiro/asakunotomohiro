...
コメント: pgp@securemail	←☆必要なのか不明だが、私が他のPGPメールアカウントと区別できるコメントを付けた。
...
※これを使わなければ、サーバ側で削除する(仕組みを仕込んでいる)。  
...
1. 上記のURLからチャット画面に入る。  
チャット入力画面：`/nick asakunotomohiro`  
チャット出力画面：**-NickServ- You are not logged in.**  
ログインしていないのは、パスワードを入力せず、ゲストとして入ったことにより、このメッセージが出てくる。  
...
また、すでに存在するユーザ名の場合は、エラーになる。  
```text
22:06:44 asakunotomohiro is now known as YourNick
22:06:44 -NickServ- This nickname is registered. Please choose a different nickname, or identify via /msg NickServ IDENTIFY 別のユーザ名 <password>
22:07:47 -NickServ- YourNick is already registered.	←☆これも出てきたメッセージの一部かな(昔のことなので分からない)。
```
このメッセージが出たときは驚いた。  
私のアカウント名はかぶらないと思う。。。  
と言うことで、別ブラウザでやり直したら問題なく通った(焦ったよ)。  

1. 認証コマンド実施。  
チャット入力画面：`/msg NickServ VERIFY REGISTER asakunotomohiro [certification]`  
チャット出力画面：  
```text
[22:28:28] <asakunotomohiro> VERIFY REGISTER asakunotomohiro [certification]
