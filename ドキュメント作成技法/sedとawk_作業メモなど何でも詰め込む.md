...
<details><summary>展開：edコマンド作業</summary>

以下、edコマンド作業。
```terminal
$ cat targetFile.md	←☆ファイル内容表示。
# 本日は晴天なり。
俺が俺にオンデマンド
本日は 雨天, なり9。
本日は 曇天, なり3。
...
以上。
$ ed targetFile.md	←☆edエディタでファイルを開く。
182	←☆文字数を表示し、最終行に移動する。
p	←☆現在行の表示。
以上。
1	←☆1行目に移動。
# 本日は晴天なり。
...
?
q
$ cat targetFile.md	←☆どういうこと(上記で「つもり」と書いたのは、保存されていないから)？
# 本日は晴天なり。
俺が俺にオンデマンド
本日は 雨天, なり9。
本日は 曇天, なり3。
...
<details><summary>展開：sedコマンド作業</summary>
...
以上。
$ sed -e 's/本日は/来年は/' targetFile.md	←☆これだけでグローバル置換になっている。
# 来年は晴天なり。
俺が俺にオンデマンド
来年は 雨天, なり9。
来年は 曇天, なり3。
...
<details><summary>展開：入出力ファイルを同じにすべからず</summary>

入力ファイルに書き出した場合、ファイル内容が無くなるため気をつけること。  
以下、その証明。
```terminal
$ cat targetFile.md
# 本日は晴天なり。
...
<details><summary>展開：複数の命令実行(セミコロン区切り)。</summary>
...
<details><summary>展開：複数の命令実行(-eオプション利用)。</summary>
...
<details><summary>展開：半角スペース区切り1つ目出力例。</summary>
...
$
```
区切ることが出来なければ、1行がまるごと出てくる(ように見える)。  

</details>

<a name="sedawkOreillyBook920004awkusecommandlineregoneline"></a>
<details><summary>展開：正規表現にヒット行のみ出力。</summary>
...
<details><summary>展開：正規表現にヒット行を加工後出力(第1弾)。</summary>
...
<details><summary>展開：正規表現にヒット行を加工後出力(第2弾)。</summary>
...
<details><summary>展開：セミコロン区切りの出力。</summary>
...
<details><summary>展開：正規表現にヒット行を加工後出力。</summary>
...
<details><summary>展開：複数の命令実行。</summary>
...
以上。
$ awk -F, '{print $1; print $2; print $3; }' targetFile.md
# 本日は晴天なり。
...
$ awk '{print $1; print $2; print $3; }' targetFile.md
#
本日は晴天なり。
...
<details><summary>展開：スクリプトファイル利用時の失敗。</summary>
...
<details><summary>展開：スクリプトファイルにオプション埋め込み失敗。</summary>
...
以下、awkプログラムをファイルに書き出し、それを実行させたプログラム。
```terminal
$ cat targetFile.md
# 本日は晴天なり。
俺が俺にオンデマンド
...
以上。
$ cat awk_shell	←☆シェルプログラムファイル(?)。
#! /bin/sh

awk '{
		print $3 ", " $1	←☆"本日は ＸＸ, なりX。"の先頭と末尾を入れ替える。
	}' |
sort |	←☆並び替え。
sed 's/。//' |	←☆置換(。記号を削除)。
awk -F, '{	←☆1行分を出力後に、末尾に。記号を付ける。
		print $0 "。"
	}'
$ sed -n 's/..,/晴れ,/p' targetFile.md | sh awk_shell	←☆-nオプションで処理対象行のみ出力させている。
なり3, 本日は。
なり5, 本日は。
なり9, 本日は。
$ sed 's/..,/晴れ,/p' targetFile.md | sh awk_shell	←☆出力抑止しない結果。
, 。
, #。
, 以上。
, 本日は。
...
$
```
awk部分をファイルに書き出したと言うより、シェルに書き出して、シェルの機能を使っているというのが正しいだろうな。  
