...
通常は、[Firefox](https://www.mozilla.org/ja/firefox/)ブラウザを使っている。  
そのユーザーエージェントを変更するには、<https://support.mozilla.org/en-US/kb/how-reset-default-user-agent-firefox>に従う必要があるようだ。  
変更方法の詳細：<https://www.howtogeek.com/113439/how-to-change-your-browsers-user-agent-without-installing-any-extensions/>  
英語なのでちんぷんかんぷん(英語力は大事だね)。  
...
#### Webブラウザからの送受信
普段[Firefox](#operatingsystemnetwork_internetsurfing)ブラウザを使っている。  
...
* 設定(以下以外はデフォルト)  
  ...
  * 一般  
    ...
    * ネットワークとディスク領域  
      * ディスク領域  
        ...
        しかし、削除や変更した場合、迷惑メールや受信不良だけにとどまらず、そもそもサーバ側で受信拒否される可能性があるようだ。  
        * 現在の正式な情報確認は、以下2種類にて可能(OS・ブラウザやそれらのバージョン情報)。  
          ...
          で、Webブラウザから送信した場合は、この情報が付かない？  
          * <https://whatmyuseragent.com>  
          * <https://browserspy.dk/browser.php>  
        * **general.useragent.override**設定値  
          **Thunderbird/2023.04.09**という値を使うことにする。  
  * 編集  
    * スペルチェック  
      すべてのチェックを外す(英語だけチェックされても困る)。  
    * 送信テキスト形式  
  ...
  * チャット  
    **[IRC](https://libera.chat)**・**[Matrix](https://matrix.org)**・**[Odnoklassniki](https://ok.ru)**・**[XMPP](https://xmpp.org)**の4種類が使えるようだ。  
